...
def merge_info(df):
    questions = pd.read_csv('loop-merge/questions.csv')
    questions['question_id'] = questions.cue + ':' + questions.ftype + ':' + \
                            questions.qid.astype(str) + ':' + questions.response
...
def main():
    ...
    v1 = pd.read_csv(Path(data_dir, 'mwpfeatures.csv'), skiprows = [1,])
    v1 = remove_practice_cols(v1)
    v1 = compile_question_types(v1, Path(info_dir, 'questions.csv'))

    v2 = pd.read_csv(Path(data_dir, 'mwpfeatures-sub2.csv'), skiprows = [1,])
    v2 = remove_practice_cols(v2)
    v2 = compile_question_types(v2, Path(info_dir, 'questions-sub2.csv'))

    v3 = pd.read_csv(Path(data_dir, 'mwpfeatures-sub3.csv'), skiprows = [1,])
    v3 = remove_practice_cols(v3)
    v3 = compile_question_types(v3, Path(info_dir, 'questions-sub3.csv'))
...
if __name__ == '__main__':
    df = main()
    df.to_csv('knowledge_type.csv', index = False)
