...
#' Moves row names to mask_type and gathers feat_type
...
pic_error <- list(
  ...
  nonvisual = c(nomask = 0.04, mask = 0.07)
) %>% clean
...
more explicitly requires visual imagery, show big effects of the mask, and put
it as Experiment 1. I think it will allow us to leverage the nuance of the
property verification experiments: they demonstrate that the disruption is
...
digraph {
  question -> cue;
  cue -> prompt [label="answer proposition"];
  cue -> picture [label="verify picture"];
...
findings from the other experiments.
  ...
  3.  It encourages people to visualize the objects. As I said, the goal of this
      ...
      effect. The remaining experiments systematically remove all incentives to
      visualize. In Experiment 2, we remove the picture trials, and in Experiment 3
      we remove the benefit of visualizing altogether.
