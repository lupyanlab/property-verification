...
from cue_first_trials import write_trials
...
class Experiment(object):
    ...
    def _initialize_display(self):
        ...
        self.stim['fix'] = visual.TextStim(self.win, text='+', height=100,
                                           color='black')
        self.stim['text'] = visual.TextStim(self.win, height=40, pos=[0,0],
    ...
    def _show_instructions(self):
        ...
        for page in instructions['pages']:
            ...
            if page == 4:
                self.stim['text'].setText('pineapple --> Does it have a seat?')
                self.stim['text'].draw()
                advance_keys = ['j',]

            elif page == 5:
                self.stim['text'].setText('apple --> Is it green?')
                self.stim['text'].draw()
                advance_keys = ['f',]
    ...
    def _present_trial(self, trial):
        ...
        self.stim['text'].setText(trial['question'])
        ...
        # show frames
        self.win.flip()
        core.wait(self.version['task']['cue_offset_to_question_onset'])
        ...
        if response == 'timeout':
            ...
            core.wait(self.version['task']['iti'])
...
if __name__ == '__main__':
    ...
    exp = Experiment(exp_dir=root_dir, version_file='cue_first.yaml')
    exp.run()
    exp.end()
