...
# Create theme elements that can be used in multiple plots
...
scale_shape_exp_run <- scale_shape_manual("Run", labels = run_labels, values = c(1, 19, 17))
scale_linetype_exp_run <- scale_linetype_manual("Run", labels = run_labels, values = c("solid", "longdash", "dotdash"))
subj_xlim <- c(0.5, nrow(subjs) + 2.5)
subj_rt_ylim <- c(min(subjs$rt) - 100, max(subjs$rt) + 200) %>%
  round(digits = -1)
subj_error_ylim <- c(0, max(subjs$error) + 0.1) %>%
  round(digits = 1)
label_size = 3
...
z_max <- abs(max(subjs_parallel_z$z_score)) + 0.4
...
ggplot(subjs_parallel_z, aes(x = measure, y = z_score, color = subj_id)) +
  ...
  coord_cartesian(ylim = c(-z_max, z_max)) +
  scale_linetype_exp_run +
  base_theme +
  guides(color = "none") +
  theme(legend.position = "top") +
...
# ---- run1-plot
run1_title <- "Effect of mask on error by feature type\n(first run)"
error_bar_plot(run1_data) + ggtitle(run1_title)
error_bar_subj_plot(run1_data) + ggtitle(run1_title)
...
# ---- run2-plot
run2_title <- "Effect of mask on error by feature type\n(second run)"
error_bar_plot(run2_data) + ggtitle(run2_title)
error_bar_subj_plot(run2_data) + ggtitle(run2_title)
