...
subj_rts <- ggplot(subjs, aes(x = rank_rt, y = rt, label = subj_id, color = subj_id)) +
  ...
  coord_cartesian(xlim = c(0.5, 49), ylim = c(500, 1500)) +
...
subj_err <- ggplot(subjs, aes(x = rank_err, y = err, label = subj_id, color = subj_id)) +
  geom_point() +
  geom_text(hjust = 0, vjust = -0.5, angle = 45, size = 4) +
  coord_cartesian(xlim = c(0.5, 49), ylim = c(0.0, 0.75)) +
...
# Notes
# -----
# MWPF129 is a massive outlier in terms of error

ggsave("plots/cue_first/descriptives/subj-rts.png", subj_rts, width = 10, height = 6)
ggsave("plots/cue_first/descriptives/subj-err.png", subj_err, width = 10, height = 6)
