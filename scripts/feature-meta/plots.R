...
base_theme <- theme(
  ...
  axis.title = element_text(vjust = 0.3, size = 6),
...
discrete_plot <- ggplot(acc_means, aes(x = feat_type, y = is_correct, fill = mask_type)) +
  ...
  annotate("text", x = 0.6, y = 0.98, label = "A", size = 3) +
  base_theme + theme(
    legend.position = c(0.8, 0.96)
  )
discrete_plot

pdf("~/Desktop/feature-last.pdf", width = 4.5, height = 5)
discrete_plot
dev.off()
...
df_first <- read.csv("./feature-first/feature-first-final.csv", stringsAsFactors = FALSE)

mod <- glmer(is_correct ~ feat_c * mask_c + (1|subj_id), data = df_first, family = binomial)
x_preds <- unique(df[,c("feat_type", "feat_c", "mask_type", "mask_c")])
y_preds <- predictSE(mod, x_preds, type = "response", se.fit = TRUE)
acc_mean_se <- cbind(x_preds, y_preds) %.%
  mutate(
...
first_discrete_plot <- ggplot(acc_means, aes(x = feat_type, y = is_correct, fill = mask_type)) +
  geom_bar(aes(fill = mask_type), stat = "identity", position = position_dodge(width = 0.6),
           color = "black", width = 0.6, size = 0.3) +
  geom_bar(aes(y = is_correct - 0.01, fill = mask_type), stat = "identity", position = position_dodge(width = 0.6),
           width = 0.59) +
...
pdf("~/Desktop/feature-first.pdf", width = 4.5, height = 5)
first_discrete_plot
dev.off()
...
continuous_estimates <- ggplot() +
  ...
  scale_y_continuous("Accuracy", breaks = seq(0,1,by=0.05), labels = percent) +
  scale_x_continuous("Amount of visual knowledge required", 
  ...
  annotate("text", x = 0.2, y = 0.98, label = "B", size = 3) +
  base_theme +
  theme(
...
pdf("~/Desktop/feature-first-continuous.pdf", width = 4.5, height = 5)
continuous_estimates
dev.off()
...
width_one_column <- 3.42 # units = "in"
width_one_point_five_columns <- 4.5 # units = "in"
width_two_columns <- 7 # units = "in"

#pdf("~/Submissions/PNAS/2014/art/fig3.pdf", width = width_one_column, height = 5)
stacked
#dev.off()


# ------------------------------------------------------------------------------
# Continuous nonvisual

question_means <- df %.% 
  group_by(facts_mean = cut(facts_mean, breaks = seq(0, 3.5, by = 0.25), labels = seq(0.125, 3.375, by = 0.25), right = FALSE), mask_type) %.%
  summarize(
    obs = n(),
    se = sd(is_correct, na.rm = TRUE) / sqrt(obs),
    is_correct = mean(is_correct, na.rm = TRUE)
  ...
  mutate(
    facts_mean = as.numeric(levels(facts_mean))[facts_mean],
    mask_type = factor(mask_type, levels = c("nomask", "mask"))
  )

mod_imag_acc <- glmer(is_correct ~ facts_mean * mask_c + (1|subj_id), data = df, family = binomial)
x_preds <- expand.grid(facts_mean = seq(min(df$facts_mean), max(df$facts_mean), by = 0.05),
                       mask_c = c(-0.5, 0.5))
y_preds <- predictSE(mod_imag_acc, x_preds, type = "response", se = TRUE)
preds <- cbind(x_preds, y_preds) %.%
  mutate(
...
continuous_estimates <- ggplot() +
  geom_smooth(aes(x = facts_mean, y = is_correct, color = mask_type, 
                  ymin = is_correct - se.fit, ymax = is_correct + se.fit, linetype = mask_type), data = preds, stat = "identity",
              show_guide = FALSE) +
  geom_line(aes(x = facts_mean, y = is_correct, color = mask_type, 
                ymin = is_correct - se.fit, ymax = is_correct + se.fit, linetype = mask_type), data = preds, stat = "identity") +
  geom_point(aes(x = facts_mean, y = is_correct, color = mask_type, size = obs,
                 shape = mask_type), data = question_means, position = position_jitter(width = 0.06, height = 0.0)) +
  # geom_linerange(aes(x = facts_mean, y = is_correct, color = mask_type, 
  #   ymin = is_correct - se, ymax = is_correct + se), data = question_means) +
  scale_y_continuous("Accuracy", breaks = seq(0,1,by=0.05), labels = percent) +
  scale_x_continuous("Amount of factual knowledge required", 
                     breaks = seq(0, 3.5, by = 0.25), 
                     labels = c("None", "", "", "", "Little", "", "", "", "Some", "", "", "", "A Lot", "", "")) +
  scale_color_manual("", labels = c("No Interference", "Visual Interference"), values = nointer_inter) +
  scale_linetype_manual("", labels = c("No Interference", "Visual Interference"), values = c(3, 1)) +
...
pdf("~/Desktop/feature-first-facts.pdf", width = 4.5, height = 5)
continuous_estimates
dev.off()
