...
library(grid)
####################################################################################################
df <- read.csv("./data/mwpf2-2014-05-05.csv", stringsAsFactors = F)
...
ggplot(subjs, aes(x = rank_rt, y = rt, label = subj_id, color = subj_id)) +
  ...
  coord_cartesian(xlim = c(0.5, 27), ylim = c(100, 700)) +
  scale_x_continuous("Subject (Ranked)", breaks = c(1, seq(5, 25, by = 5))) +
  scale_y_continuous("Average RT (ms)") +
  theme(legend.position = "none")

# ggsave("./figures/feature_first/subj-rt.png", width = 8, height = 5, units = "in")
...
# ggsave("./figures/feature_first/subj-acc.png", width = 8, height = 5, units = "in")
####################################################################################################
...
df <- df %.%
  ...
  select(subj_id, block_ix, trial_ix, 
         ...
         mask_c, feature_c,
         imagery_z, facts_z, diff_z,
         question_id) %.%
  arrange(subj_id, block_ix, trial_ix)

# write.csv(df, file = "./data/mwpf2-final.csv", row.names = FALSE)
...
if (FALSE) {
  ...
  save("mask_rt", "mask_rt_nomask", "mask_rt_chi", "mask_rt_anova",
       file = "./models/feature_first/mask_rt.RData")
}

load("./models/feature_first/mask_rt.RData")
...
if (FALSE) {
  ...
  save("mask_acc", "mask_acc_nomask", "mask_acc_chi",
       file = "./models/feature_first/mask_acc.RData")
}

load("./models/feature_first/mask_acc.RData")
...
if (FALSE) {
  ...
  save("feature_rt", "feature_rt_nofeature", "feature_rt_chi", "feature_rt_anova",
       file = "./models/feature_first/feature_rt.RData")
}

load("./models/feature_first/feature_rt.RData")
...
if (FALSE) {
  ...
  save("feature_acc", "feature_acc_nofeature", "feature_acc_chi",
       file = "./models/feature_first/feature_acc.RData")
}
...
if (FALSE) {
  ...
  save("inter_rt", "inter_rt_nointer", "inter_rt_chi", "inter_rt_anova",
       file = "./models/feature_first/inter_rt.RData")
}

load("./models/feature_first/inter_rt.RData")
...
if (FALSE) {
  ...
  save("inter_acc", "inter_acc_nointer", "inter_acc_chi",
       file = "./models/feature_first/inter_acc.RData")
}

load("./models/feature_first/inter_acc.RData")
...
# ggsave("figures/feature_first/reactiontime.png", width = 6, height = 6, units = "in")
...
# ggsave("figures/feature_first/accuracy.png", width = 6, height = 6, units = "in")
...
# ggsave("./figures/feature_first/randomeffects_rt.png", width = 10, height = 9, units = "in")
...
if (FALSE) {
  ...
  save("imag_acc", "imag_acc_nointer", "imag_acc_chi",
       file = "./models/feature_first/imag_acc.RData")
  ...
  save("imag_acc_diff", "imag_acc_diff_nointer", "imag_acc_diff_chi",
       file = "./models/feature_first/imag_acc_diff.RData")
}

load("./models/feature_first/imag_acc.RData")

summary(imag_acc)
imag_acc_chi

load("./models/feature_first/imag_acc_diff.RData")
...
# ggsave("figures/feature_first/accuracy_imagery.png", width = 6, height = 6, units = "in")